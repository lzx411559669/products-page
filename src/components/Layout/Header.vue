<template>
  <div
    class="header-box w-full sticky top-0 flex items-center border h-16 px-2 justify-between"
  >
    <div @click="jump('/')" class="logo flex items-center">
      <div class="text-right">
        <div class="text-sm ">JINHUAN ART&CRAFT</div>
        <div class=" text-lg">金环工艺</div>
      </div>
      <div class="ml-4">
        <img
    ...
    <div class="flex items-center">
      <div @click="changeLang" class="flex items-center mr-3">
        <svg
          t="1680920210859"
          class="icon"
      ...
      <div class="flex items-center">
        ...
        class="i-material-symbols:format-list-bulleted text-2xl text-white"
      ></div>
      <div class="font-thin text-base">{{$t('菜单')}}</div>
      </div>
  ...
  >
    <div class="h-full justify-center relative">
      <div class="pt-[30%]">
        <div @click="jump(item.url)" v-for="item in menus">
          <div class="text-center my-10 text-xl">{{ $t(item.name) }}</div>
        </div>
      </div>
      <div class="absolute top-2 right-2">
        <div
          @click="showRight = false"
          class="i-material-symbols:close text-black text-3xl"
        ></div>
      </div>
    </div>
  </nut-popup>
